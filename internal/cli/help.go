...
var commandDocs = []commandDoc{
	...
	},
	{
		...
		long: "simulate answers the question impact does not: of everything a change to this symbol can reach, which parts does a test actually guard? It takes impact's affected set (callers, type consumers, data flows -- not callees, which a change cannot break) and resolves covering tests per symbol, then reports the UNCOVERED remainder.\n\n" +
			"The graph is evidence, not an oracle, so coverage is reported in three states and never as a certainty. \"confirmed\" means an inbound relation whose resolution the provider itself calls resolved (exact, import_resolved, package) -- tier \"edge\". \"heuristic\" means the edge exists but was resolved by name, by pattern, or by inferring a receiver type (tier \"edge-heuristic\"), or is a mirror-file match: real evidence, but not proof the test executes the symbol, and the case dynamic dispatch produces. \"unresolved\" means no test was RESOLVED, which is not proof no test exists -- static analysis cannot follow dynamic dispatch, reflection or generated code.\n\n" +
			"Every claim that is not confirmed appears under VERIFY with a command that settles it. ANALYSIS MAY BE PARTIAL fires when any input was incomplete -- a degraded snapshot, in-scope parse failures, a truncated impact listing, or heuristic evidence -- and names each reason; --format json exposes the same as analysis_partial plus partial_reasons.\n\n" +
